...
const getContract = async() =>{
  ...
  let contract = new ethers.Contract(
    FUNDING_CONTRACT_ADDRESS,  //after deploy
    FUNDING_CONTRACT_ABI, // after compile
    provider
);
return contract;
};
...
const getGoalAmt = async() => {
  try{
    ...
    var required = await contractObject.goalAmt();
    goalAmt= required.toString();
    gAmt = required.toNumber();
    console.log("goalAmt ",gAmt);
  }catch(error){
    console.error(error);
  }
}
 ...
 //state changes should be seen only when rendering happens
  ...
  function renderBody(){
    ...
    if(fundingEnded && success){
      return(
        <div className={styles.failure}>
          <h1>HURRY ! Funding is successfull,<br/>So you can claim the NFT reward.</h1>
